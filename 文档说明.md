...
| [application-dev.yml](keystone-admin/src/main/resources/application-dev.yml) | 本地开发：数据库 / Redis 连接信息 |
| [application-docker.yml](keystone-admin/src/main/resources/application-docker.yml) | Docker 部署：连接信息通过环境变量注入 |
...
**本地开发**只需修改 [application-dev.yml](keystone-admin/src/main/resources/application-dev.yml)，按需调整：

* 数据库连接（`url` / `username` / `password`）
* Redis 连接（`host` / `port` / `password`）
* 日志路径（`logging.file.path`）
